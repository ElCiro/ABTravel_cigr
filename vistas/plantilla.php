<?php
$blog_cigr = ControladorBlog::ctrMostrarBlog();
$categorias_cigr = ControladorBlog::ctrMostrarCategorias();
$articulos = ControladorBlog::ctrMostrarConInnerjoin(0, 5);
$totalAriculos = ControladorBlog::ctrMostrarTotalArticulos();

$totalPaginas = ceil(count($totalArticulos)/5);
//echo '<pre class="bg-white">'; print_r(count($)); echo '</pre>';
?>
...
<head>
	...
	if (isset($_GET["pagina"])) {
		...
		foreach ($categorias_cigr as $key => $value) {
		...
		if ($validarRuta == "categorias") {
			...
			echo '<title>' . $blog_cigr["titulo"] . ' | ' . $value["descripcion_categoria"] . '</title>
			<meta name="title" content="' . $value["titulo_categoria"] . '>">
			<meta name="description" content="' . $value["descripcion_categoria"] . '">';
			$palabras_claves = json_decode($value["p_clave_categoria"], true);
			$p_claves = "";
			...
			echo '<title>' . $blog_cigr["titulo"] . '</title>
			<meta name="title" content="' . $blog_cigr["titulo"] . '>">
			<meta name="description" content="' . $blog_cigr["descripcion"] . '">';
			$palabras_claves = json_decode($blog_cigr["palabras_claves"], true);
			$p_claves = "";
			foreach ($palabras_claves as $key => $value) {
				$p_claves .= $value . ", ";
			}
...
<body>
	...
	if (isset($_GET["pagina"])) {
		 ...
		 * limit 5,5 (5,6,7,8,9) pagina 2 -1*5=5
		 * limit 10, 5(11,12,13,14) pagina 3 3-1 * =10
		 */
		...
		/**Validar las rutas */
		if ($validarRuta != "categorias_cigr") {
